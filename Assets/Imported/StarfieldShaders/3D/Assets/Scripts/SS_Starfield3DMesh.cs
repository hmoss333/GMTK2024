...
public static class SS_Starfield3DMesh
{
    ...
    {
        _detail = Mathf.Clamp(_detail, 1, 24);
        ...
        for (int k = 0; k < 6; k++)
        {
            ...
            Vector3 forward = rotation * Vector3.forward * 1.75f;
            ...
            {
                -right,
                right * Mathf.Cos(-Mathf.PI / 3f) + up * Mathf.Sin(-Mathf.PI / 3f),
                right * Mathf.Cos(Mathf.PI / 3f) + up * Mathf.Sin(Mathf.PI / 3f),
            };

            for (int i = 0; i < 6; i++)
            {
                Vector3 center = right * Mathf.Cos(Mathf.PI * i / 3f) + up * Mathf.Sin(-Mathf.PI * i / 3f);
                rotation = Quaternion.AngleAxis(i * 60f, forward);
    ...
    private static void CreateDetail(int _detail, int _i, List<Vector3> _vertices, List<int> _triangles, ref List<Vector3> realVertices, ref List<int> realTriangles, ref List<Vector3> normals)
    {
        ...
        Vector3 center = first + (a + b) / 3f;
        Vector3 offset1 = first - center;
        Vector3 offset2 = first + a - center;
        Vector3 offset3 = first + b - center;

        center = first + 2 * (a + b) / 3f;
        Vector3 offset4 = first + a - center;
        Vector3 offset5 = first + b + a - center;
        Vector3 offset6 = first + b - center;
