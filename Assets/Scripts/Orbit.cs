...
public class Orbit : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            launched = true;
            GameObject earth = celestials.FirstOrDefault(x => x.name == "Earth");
            GameObject ship = Instantiate(shipPrefab, earth.transform.position, Quaternion.identity);
            celestials.Add(ship);

            Camera.main.transform.parent = ship.transform;
            Camera.main.transform.localPosition = new Vector3(0, 18.9f, -50f);

            float m2 = ship.GetComponent<Rigidbody>().mass;
            float r = Vector3.Distance(ship.transform.position, ship.transform.position);
            ...
            if (IsElipticalOrbit)
            {
                ...
                ship.GetComponent<Rigidbody>().velocity += ship.transform.right * Mathf.Sqrt((G * m2) * ((2 / r) - (1 / (r * 1.5f))));
            }
            else
            {
                ...
                ship.GetComponent<Rigidbody>().velocity += ship.transform.right * Mathf.Sqrt((G * m2) / r);
            }
        }
    }
